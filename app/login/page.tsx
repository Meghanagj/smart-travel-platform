...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 2. Baseline Password Presence Check
    if (!password || password.length < 6) {
      const passMsg = "Password must be at least 6 characters long.";
      setError(passMsg);
      toast.error(passMsg);
      return;
    }
    ...
    try {
      ...
      // Simulating downstream token handshaking layer
      await new Promise((resolve) => setTimeout(resolve, 1200));

      console.log("Authentication handshake successful for:", { email: email.trim() });
      ...
      toast.error("Invalid credentials or server context unreachable.");
      setLoading(false);
    }
  };
    ...
    <main className="min-h-[calc(100vh-4rem)] bg-slate-50 flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="sm:mx-auto w-full max-w-md relative z-10">
        ...
        <div className="bg-white py-8 px-6 sm:px-10 rounded-2xl shadow-md border border-slate-100">
          ...
          {error && (
            <div role="alert" className="mb-5 p-3 bg-red-50 border border-red-100 rounded-xl flex items-start gap-2 text-red-600 text-xs sm:text-sm font-semibold animate-fadeIn">
          ...
          <form className="space-y-5" onSubmit={handleSubmit} noValidate>
            ...
            <div>
              <div className="flex justify-between items-center mb-1.5">
                ...
                <Link href="/forgot-password" className="text-xs font-bold text-blue-600 hover:text-blue-700 transition-colors focus:outline-none focus:underline">
                  Forgot Password?
                </Link>
              </div>
              <div className="relative">
                <input
                  ...
                  className="w-full border border-slate-200 p-3 pr-12 rounded-xl text-slate-800 placeholder-slate-400 font-medium focus:outline-none focus:ring-4 focus:ring-blue-100 focus:border-blue-500 transition-all text-sm md:text-base bg-slate-50/50 disabled:opacity-60 disabled:cursor-not-allowed"
                />
                <button
                  type="button"
                  disabled={loading}
            ...
            <div className="pt-2">
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3.5 px-4 rounded-xl shadow-md transition-all text-sm sm:text-base focus:outline-none focus:ring-4 focus:ring-blue-300 disabled:opacity-50"
              >
