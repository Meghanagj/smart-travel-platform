...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name.trim()) {
      const nameMsg = "Please enter your full name.";
      setError(nameMsg);
      toast.error(nameMsg);
      return;
    }
    ...
    if (!strongPasswordRegex.test(formData.password)) {
      const passMsg = "Password must be at least 6 characters and include uppercase, lowercase, and numbers.";
      setError(passMsg);
      toast.error(passMsg);
      return;
    }
    ...
    try {
      ...
      console.log("Registration schema saved successfully:", { ...formData, email: formData.email.trim() });
    ...
    <main className="min-h-[calc(100vh-4rem)] bg-slate-50 flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="sm:mx-auto w-full max-w-md relative z-10">
        ...
        <div className="bg-white py-8 px-6 sm:px-10 rounded-2xl shadow-md border border-slate-100">
          ...
          {error && (
            <div role="alert" className="mb-4 p-3 bg-red-50 border border-red-100 rounded-xl flex items-start gap-2 text-red-600 text-xs sm:text-sm font-semibold animate-fade-in">
          ...
          <form className="space-y-5" onSubmit={handleSubmit} noValidate>
            <div>
              ...
              <input
                ...
                className="w-full border border-slate-200 p-3 rounded-xl text-slate-800 placeholder-slate-400 font-medium focus:outline-none focus:ring-4 focus:ring-blue-100 focus:border-blue-500 transition-all text-sm md:text-base bg-slate-50/50"
              />
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="At least 6 characters with upper, lower & numbers"
                  value={formData.password}
                  onChange={(e) => setFormData(prev => ({ ...prev, password: e.target.value }))}
                  className="w-full border border-slate-200 p-3 pr-12 rounded-xl text-slate-800 placeholder-slate-400 font-medium focus:outline-none focus:ring-4 focus:ring-blue-100 focus:border-blue-500 transition-all text-sm md:text-base bg-slate-50/50"
                />
                <button
                  type="button"
                  disabled={loading}
            ...
            <div className="pt-2">
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3.5 px-4 rounded-xl shadow-md transition-all text-sm sm:text-base focus:outline-none focus:ring-4 focus:ring-blue-200 disabled:opacity-50"
              >
